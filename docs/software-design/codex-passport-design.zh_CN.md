...
3. **消息**：任务卡片，高亮卡片作为语音回复目标。
4. **设置**：提示音开关与音量。
5. **二维码**（叠加层）：主页地址；首页或额度页按 `OK` 开关。
...
- `UP`：上一页（若在二维码则先关闭）。
- `DOWN`：下一页（若在二维码则先关闭）。
- `OK`：选择下一条消息、调整设置页音量，或在首页/额度页切换二维码。
- 在高亮消息上长按 `OK` 录音。完整 UUID 与消息快照同时复制，并冻结到本次语音操作结束。操作说明见[语音回复](../../projects/codex-passport/README.zh_CN.md#语音回复)。
...
`passport_voice_messages_page_t` 将显示卡片和完整 UUID 放在同一个快照中。`passport_voice.h` 定义录音上限、操作和结果状态。音频按块确认，每块使用已有的有界分片格式，分片大小依据 MTU。主机与固件拒绝非法长度、序号变化和过期事务 ID。相同音频块重试具有幂等性；确认的文字只提交一次，重连后不重放。

`tools/passport_voice.py:VoiceHost` 管理本地转写、预览文字和提交状态，`tools/codex_bridge.py:deliver` 管理桌面任务投递。临时音频不写入 NVS，转写完成后不保留。
### 6.3 NVS 持久化存储
