...
- 修复主机定时同步用量时覆写设备端静音与音量保存设置的问题。
- 修复 macOS 蓝牙状态读取与通知竞争导致的语音丢包；恢复音频逐块确认和五秒静音阈值，增加录音计时、上传进度及投递回执指引。
- Codex Passport 消息页用 UP/DOWN 在卡片和分页之间来回移动，可以从第 2 页回到第 1 条。语音提示按阶段说明：等 Speak now 再说话，短按 OK 结束录音，核对文字后再短按 OK 发送。
- 新增 Codex Passport 双击说话回复：绑定任务录音、本地转写、设备端预览与确认发送、投递回执，以及断连和重复提交保护。
