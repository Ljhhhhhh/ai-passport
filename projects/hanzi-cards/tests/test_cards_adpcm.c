...
static uint8_t encode_nibble(cards_adpcm_state_t *state, int16_t sample)
{
...
int main(void)
{
    ...
    uint8_t blob[sizeof(cards_adpcm_header_t) + sizeof(packed)];
    cards_adpcm_state_t encode_state;
    cards_adpcm_state_t decode_state;
    ...
    cards_adpcm_init(&encode_state, 0, 0);
    ...
    header.magic = CARDS_ADPCM_MAGIC;
    ...
    assert(cards_adpcm_parse_clip(blob, sizeof(blob), &header, &data, &data_len));
    cards_adpcm_init(&decode_state, header.predictor, header.step_index);
    assert(cards_adpcm_decode_bytes(&decode_state, data, data_len, 0, decoded,
                                    sizeof(decoded) / sizeof(decoded[0])) ==
           sizeof(decoded) / sizeof(decoded[0]));
    assert(memcmp(decoded, expected, sizeof(expected)) == 0);

    assert(cards_player_start(&player, blob, sizeof(blob)));
    assert(cards_player_next_chunk(&player, chunk, 4) == 4);
    assert(cards_player_status(&player) == CARDS_PLAYER_PLAYING);
    cards_player_cancel(&player);
    assert(cards_player_status(&player) == CARDS_PLAYER_CANCELLED);
    assert(cards_player_next_chunk(&player, chunk, 4) == 0);

    assert(cards_player_start(&player, blob, sizeof(blob)));
    size_t total = 0;
    while (cards_player_status(&player) == CARDS_PLAYER_PLAYING) {
        total += cards_player_next_chunk(&player, chunk, 3);
    }
    assert(total == header.sample_count);
    assert(cards_player_status(&player) == CARDS_PLAYER_DONE);

    blob[0] ^= 0xFF;
    assert(!cards_player_start(&player, blob, sizeof(blob)));
    assert(cards_player_status(&player) == CARDS_PLAYER_FAILED);

    cards_model_init(&model);
    cards_model_on_button(&model, CARDS_BTN_DOWN);
    uint32_t stale = model.play_gen;
    cards_model_on_button(&model, CARDS_BTN_DOWN);
    assert(!cards_model_play_current(&model, stale));
