...
int cards_player_start(cards_player_t *player, const uint8_t *blob, size_t blob_len)
{
    ...
    if (!cards_adpcm_parse_clip(blob, blob_len, &player->header, &data, &data_len)) {
        player->status = CARDS_PLAYER_FAILED;
        return 0;
    }

    cards_adpcm_init(&player->state, player->header.predictor, player->header.step_index);
    player->data = data;
    player->data_len = data_len;
    player->nibble_index = 0;
    player->status = CARDS_PLAYER_PLAYING;
    return 1;
}
...
void cards_player_cancel(cards_player_t *player)
{
    ...
    player->status = CARDS_PLAYER_CANCELLED;
}
...
size_t cards_player_next_chunk(cards_player_t *player, int16_t *dst, size_t dst_samples)
{
    ...
    if (!player || player->status != CARDS_PLAYER_PLAYING || !dst || dst_samples == 0U) {
        return 0;
    }

    remaining = player->header.sample_count - player->nibble_index;
    if (remaining == 0U) {
        player->status = CARDS_PLAYER_DONE;
        return 0;
    }
    ...
    produced = cards_adpcm_decode_bytes(&player->state, player->data, player->data_len,
...
{
    return player ? player->status : CARDS_PLAYER_FAILED;
}
