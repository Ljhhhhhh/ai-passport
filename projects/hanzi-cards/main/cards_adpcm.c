...
_Static_assert(sizeof(cards_adpcm_header_t) == 12U, "ADPCM clip header must stay stable");
                                ...
                                size_t nibble_index, int16_t *dst, size_t dst_samples)
{
    ...
    if (!state || !src || !dst || dst_samples == 0U) {
        return 0;
    }
    ...
    while (produced < dst_samples) {
        ...
        dst[produced++] = cards_adpcm_decode_nibble(state, nibble);
        nibble_index++;
    }
    return produced;
}
...
{
    cards_adpcm_header_t parsed;
    size_t packed_len;

    if (!blob || blob_len < sizeof(cards_adpcm_header_t) || !header || !data || !data_len) {
        return 0;
    }

    memcpy(&parsed, blob, sizeof(parsed));
    if (parsed.magic != CARDS_ADPCM_MAGIC || parsed.sample_count == 0U) {
        return 0;
    }
    ...
    if (blob_len < sizeof(cards_adpcm_header_t) + packed_len) {
        return 0;
    }

    *header = parsed;
    *data = blob + sizeof(cards_adpcm_header_t);
    *data_len = packed_len;
    return 1;
}
